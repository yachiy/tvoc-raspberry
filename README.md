...
-   **動的なデータ取得間隔**: センサーからのデータ取得に成功した場合は5分間隔、失敗した場合は10秒間隔でリトライします。
...
-   **LINE警告通知**: 
    -   **高濃度警告**: TVOCの値が設定した閾値(1.0ppm)を2時間以上継続して超えた場合、LINEで警告を送信します。
    -   **センサー異常通知**: センサーからのデータ取得に10回以上連続で失敗した場合、LINEで異常を通知します。
-   **通知クールダウン**: 各種のLINE通知は、同様の通知が2時間以内に送信されている場合はスキップされ、通知の頻発を防ぎます。
